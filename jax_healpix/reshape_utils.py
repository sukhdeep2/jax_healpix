...
def set_m_indxs(l_max, m, indxs):
    ...
    i = np.arange(l_max + 1)
    indxs[:, m] = np.where(i >= m, indxs[:, m] - (m * (m - 1) / 2) - m, -1)
    return indxs


def alm_indxs(l_max):
    """indxs to covert 1-D alms to 2-D alm array"""
    indxs = np.arange((l_max + 1) ** 2).reshape(l_max + 1, l_max + 1).T
    set_m_indxs_i = partial(set_m_indxs, l_max)
    for i in range(1, l_max + 1):  # FIXME: vectorize
        indxs = set_m_indxs_i(i, indxs)
    return indxs
...
def l_stack_mask(l_max):
    ...
    """
    m_mat = np.tile(np.arange(l_max + 1), l_max + 1).reshape(l_max + 1, l_max + 1)
    l_mat = m_mat.T
    mask = m_mat <= l_mat
    return mask
...
def m_stack_mask(l_max):
    ...
    """
    l_mat = np.tile(np.arange(l_max + 1), l_max + 1).reshape(l_max + 1, l_max + 1)
    m_mat = l_mat.T
    mask = m_mat <= l_mat
    return mask
...
def ring_pol_mask(nside, ring_i, mask):
    j = np.arange(4 * nside) + 1
    mask[ring_i - 1, :] = np.where(j <= 4 * ring_i, mask[ring_i - 1, :], False)
    mask[4 * nside - ring_i - 1, :] = np.where(
        j <= 4 * ring_i, mask[4 * nside - ring_i - 1, :], False
    )
    return mask


def stack_maps(nside, maps):
    mask = np.ones((4 * nside - 1, 4 * nside), dtype="bool")
...
def pol_ring_indxs(nside, ring_i, indxs):
    i = ring_i
    ii = np.arange(4 * nside) + 2 * i * (i + 1)
    ii = np.where(ii < 4 * (i + 1) + 2 * i * (i + 1), ii, -1)
    indxs[i, :] = ii

    s_pol_fact_i = 12 * nside**2 - 2 * i * (i + 1) - 4 * (i + 1)
    ii = np.arange(4 * nside) + s_pol_fact_i
    ii = np.where(ii < 4 * (i + 1) + s_pol_fact_i, ii, -1)

    indxs[4 * nside - i - 2, :] = ii
    return indxs
...
def eq_ring_indxs(nside, ring_i, indxs):
    ...
    indxs[i, :] = np.arange(4 * nside) + eq_fact  # +2*i*(i+1)
    return indxs


def reshape_maps(nside, maps):
    indxs = np.zeros((4 * nside - 1, 4 * nside), dtype="int")
