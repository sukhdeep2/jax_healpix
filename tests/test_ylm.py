...
def test_spherical_harmonics_creation():
    ...
    assert ylm_calc.spins_to_compute == (0, 2) # Should be sorted
...
def test_get_ylm_spin0_y00_basic():
    # Basic sanity check for Y_0,0 which is 1/sqrt(4pi)
    l_max = 0
    beta_values = jnp.array([0.0, 0.5, 1.0]) # cos(theta) values
    ...
    # s0_ylm_std should be (l_max+1, l_max+1, n_beta) = (1, 1, 3)
    assert s0_ylm_std.shape == (1, 1, len(beta_values))
    expected_y00 = 1.0 / jnp.sqrt(4 * jnp.pi)
    assert jnp.allclose(s0_ylm_std[0, 0, :], expected_y00)

    # Log recurrence (should also yield same for Y00)
    ...
    # Log abs should be log(expected_y00), sign should be 1
    assert jnp.allclose(s0_ylm_log_abs[0, 0, :], jnp.log(expected_y00))
    assert jnp.all(s0_ylm_log_signs[0, 0, :] == 1)

def test_get_ylm_caching():
    l_max = 2
    beta_values = jnp.array([0.5])
    ylm_calc = SphericalHarmonics(l_max=l_max, spins_to_compute=(0,))

    ylm1 = ylm_calc.get_ylm(spin=0, beta_values=beta_values)
    ylm2 = ylm_calc.get_ylm(spin=0, beta_values=beta_values) # Should be cached

    assert jnp.array_equal(ylm1, ylm2)

    # Test with log recurrence caching
    ylm_calc_log = SphericalHarmonics(l_max=l_max, spins_to_compute=(0,), use_log_recurrence=True)
...
def test_stack_ylm_2d_to_1d_single_array(order):
    l_max = 2
    # Mock YLM array: (l_max+1, l_max+1, n_beta) = (3, 3, 2)
    ...
    num_coeffs = (l_max + 1) * (l_max + 2) // 2 # For l_max=2, this is 6
    assert stacked_ylm.shape == (num_coeffs, 2) # (num_coeffs, n_beta)

    # Y_0,0 element: (l=0,m=0)
    assert jnp.array_equal(stacked_ylm[0,:], ylm_2d_data[0,0,:])

    if order == 'l': # L-primary order: (0,0), (1,0), (1,1), (2,0), (2,1), (2,2)
        # Stacked indices:    0,     1,     2,     3,     4,     5
        # (l,m) pairs:      (0,0), (1,0), (1,1), (2,0), (2,1), (2,2)
        assert jnp.array_equal(stacked_ylm[1,:], ylm_2d_data[1,0,:]) # Y_1,0
        assert jnp.array_equal(stacked_ylm[2,:], ylm_2d_data[1,1,:]) # Y_1,1
        assert jnp.array_equal(stacked_ylm[3,:], ylm_2d_data[2,0,:]) # Y_2,0
        assert jnp.array_equal(stacked_ylm[4,:], ylm_2d_data[2,1,:]) # Y_2,1
        assert jnp.array_equal(stacked_ylm[5,:], ylm_2d_data[2,2,:]) # Y_2,2
    elif order == 'm': # M-primary order (Healpy): (0,0), (1,0), (2,0), (1,1), (2,1), (2,2)
        # Stacked indices:    0,     1,     2,     3,     4,     5
        # (l,m) pairs:      (0,0), (1,0), (2,0), (1,1), (2,1), (2,2)
        assert jnp.array_equal(stacked_ylm[1,:], ylm_2d_data[1,0,:]) # Y_1,0
        assert jnp.array_equal(stacked_ylm[2,:], ylm_2d_data[2,0,:]) # Y_2,0
        assert jnp.array_equal(stacked_ylm[3,:], ylm_2d_data[1,1,:]) # Y_1,1
        assert jnp.array_equal(stacked_ylm[4,:], ylm_2d_data[2,1,:]) # Y_2,1
        assert jnp.array_equal(stacked_ylm[5,:], ylm_2d_data[2,2,:]) # Y_2,2
...
def test_stack_ylm_2d_to_1d_dict_input(order):
    l_max = 1
    # Mock YLM dict: {spin: array(l_max+1, l_max+1, n_beta)}
    ylm_dict_2d = {
    ...
    num_coeffs = (l_max + 1) * (l_max + 2) // 2 # For l_max=1, this is 3
    ...
    ylm_dict_bad_shape = {0: jnp.ones((l_max, l_max, 1))}
    with pytest.raises(ValueError):
